...
import (
	...
	Cevermos "apigoevermos/src/apievermos/controllers/evermos"
	// Cevermos "apibafevermos/controllers/dev/evermos/orderdata"
	...
	_ "apigoevermos/src/apievermos/docs"
	// _ "apibafevermos/docsdev"
	// _ "apibafgate/tmp"
)
...
func main() {
	...
	url := ginSwagger.URL(host + addr + "/api/v1/apievermos/swagger/doc.json") // The url pointing to API definition

	v1 := router.Group("/api/v1/apievermos")
	{
		//Modul evermos
		...
		v1.POST("evermos/product/list", Cevermos.GetProduct)
		v1.POST("evermos/product/insert", Cevermos.InsertProduct)

		v1.GET("/status", Cevermos.CheckReadiness)
	...
	go func() {
		<-quit
		logger.Println("Server apievermos is shutting down...")
		...
		if err := server.Shutdown(ctx); err != nil {
			logger.Fatalf("Could not gracefully shutdown the server apievermos: %v\n", err)
		}
		close(done)
	}()

	logger.Println("Server apievermos is ready to handle requests at", subhost, addr)
	...
	<-done
	logger.Println("Server apievermos stopped")
...
func LoggingActivity() {
	...
	var filename string = "../apilogevermos/logevermos/log" + date + ".log"
